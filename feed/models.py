...
class Assignment(models.Model):
    ...
    custom_criteria = models.TextField(
        ...
        blank=True,
        default='',
        help_text='Власні детальні критерії та шкала оцінювання саме для цього завдання (враховуються вчителем та ШІ при перевірці)'
    )
    no_submission_required = models.BooleanField(
        'Не вимагає здачі робіт (завдання для опрацювання / усне)',
        default=False,
    ...
    def get_default_gr_list(self):
        ...
        if not self.default_ai_grs:
            return []
        try:
            import json as _json
            data = _json.loads(self.default_ai_grs)
...
- Прикріплені файли вчителя (презентації .pptx/.ppt/.odp/PDF, документи, зображення) є повноцінним першоджерелом завдання та навчального контексту уроку.
...
- Якщо у файлі є кілька завдань, але вчитель вказав виконати тільки одне конкретне (наприклад, завдання 3):
...
ПОШУК ТА РОЗПІЗНАВАННЯ ЗАВДАННЯ В ПРЕЗЕНТАЦІЯХ, PDF ТА ЗОБРАЖЕННЯХ ВЧИТЕЛЯ:
...
- Якщо вчитель дозволив учням обрати будь-яке завдання з файлу умови чи презентації:
  ...
  3. Якщо учень НЕ вказав обране завдання:
     * Автоматично зістав здану роботу із завданнями файлу умови чи слайдів презентації та визнач найбільш імовірне завдання.
  ...
  4. Якщо ШІ взагалі не зрозумів, яке завдання виконано, або робота не відповідає жодному завданню:
     * ⚠️ УВАГА: спочатку перевір усі слайди презентацій, PDF та зображення вчителя. Лише якщо зміст ДІЙСНО не підходить під жодне завдання з опису, слайдів чи файлів:
...
class AISettings(models.Model):
    ...
    @property
    def temperature_dot(self):
        try:
            val = float(self.temperature)
            return f"{val:.2f}".rstrip('0').rstrip('.') if val != 0 else "0.0"
        except (ValueError, TypeError):
            return "0.2"
